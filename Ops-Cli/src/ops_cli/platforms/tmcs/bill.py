...
def _write_template(*, export_scene: dict[str, Any], query_scene: dict[str, Any]) -> Path:
    template = {
        "site": TMCS_SITE,
        "scenes": {
        ...
        "download_query": {
            "url": query_scene.get("url"),
            "method": query_scene.get("method"),
            "headers": _sanitize_tmcs_headers(query_scene.get("headers") or {}, query_scene.get("cookies") or []),
...
def _download_bill_file(*, headers: dict[str, str], query_scene: dict[str, Any], bill_code: str, output_dir: Path) -> Path:
    ...
    try:
        ...
        query_headers = dict(query_scene.get("headers") or headers)
        file_url = None
        for _ in range(24):
...
def _download_statement_list(*, headers: dict[str, str], export_scene: dict[str, Any], query_scene: dict[str, Any], start: date, end: date, output_dir: Path) -> Path:
    ...
    _, nested_payload, nested_content = tmcs_download(gei_url, headers=dict(export_scene.get("headers") or {}))
...
def learn_bill_download(*, force: bool = False) -> CommandResponse:
    inputs = {"site": TMCS_SITE, "scenes": [TMCS_BILL_EXPORT_SCENE, TMCS_BILL_QUERY_SCENE], "force": force}
    export_path = None
    query_path = None
    try:
    ...
    template_path = _write_template(export_scene=export_scene, query_scene=query_scene)
    context_path = write_runtime_context(
        ...
        artifacts=[str(export_path), str(query_path), str(template_path)],
    )
...
def run_bill_download(
    *,
    ...
    for scene_name in (TMCS_BILL_EXPORT_SCENE, TMCS_BILL_QUERY_SCENE):
        try:
