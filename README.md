...
- `tmcs`：商品同步、库存导出/调整、库存查询、月账单下载、推广账单下载、XP 工单数量读取、物流履约数据概览读取（规划中）。
...
- 猫超月账单整理、猫超商品列表更新、猫超单品 ROI 测算、猫超 XP 工单监控、猫超物流履约监控（规划中）。
